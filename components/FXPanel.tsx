  ...
  useEffect(() => {
    const fetchFX = async () => {
      ...
      if (data.NGN !== 'N/A') {
        ...
        setFx(data)
        const newFlash: any = {}
        Object.keys(data).forEach(k => { newFlash[k] = true })
        setFlash(newFlash)
      ...
      <div style={{borderTop:'1px solid #1a2d4a',paddingTop:'24px'}}>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:'8px'}}>
          {currencies.map(({ code, country, flag }) => (
            <div key={code} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'8px 12px',background:flash[code]?'#0f2a1a':'#0a1628',border:'1px solid '+(flash[code]?'#00c48c33':'#1a2d4a'),borderRadius:'6px',transition:'all 0.5s'}}>
              ...
              <div style={{fontFamily:'Space Mono,monospace',fontSize:'12px',color:flash[code]?'#ffffff':'#00c48c',fontWeight:700,transition:'color 0.5s'}}>
